...
describe('connected editor', () => {
  beforeEach(
    withRetry(async done => {
      // OSX and safari are necessary
      driver = await startSession()
      await driver.get(process.env.LOCAL_ENV ? LOCAL_URL : PROXY_URL)

      const emailField = await getElementByTag(
        driver,
        '[data-test-path="email"]'
      )
  ...
  it(
    'should toggle bold and save changes',
    withRetry(async () => {
      await sleep(300)
  ...
  it(
    'should toggle italic and save changes',
    withRetry(async () => {
      await sleep(300)
      await actions.sendKeys('the following text should be ')
  ...
  it(
    'should toggle location and save changes',
    withRetry(async () => {
      await sleep(300)
      await actions.sendKeys('the following text should be ')
  ...
  it(
    'should insert atomic source and edit source fields',
    withRetry(async () => {
      await sleep(300)
  ...
  it(
    'should test data integrity in round trip testing',
    withRetry(async () => {
      await sleep(300)
